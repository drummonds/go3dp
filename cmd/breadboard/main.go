...
func SDFBox(x, y, z float64) sdf.SDF3 {
	box, err := sdf.Box3D(v3.Vec{X: x, Y: y, Z: z}, 0)
	if err != nil {
		panic("calc error")
	}
	box = sdf.Transform3D(box, sdf.Translate3d(v3.Vec{X: x / 2, Y: y / 2, Z: z / 2}))
	return box
}
...
func endSpacer() sdf.SDF3 {
	...
	box = sdf.Transform3D(box, sdf.Translate3d(v3.Vec{X: 0, Y: baseHeight, Z: 0}))
	return box
}
...
func completeSpacer() (sdf.SDF3, error) {
	...
	box = sdf.Transform3D(box, sdf.Translate3d(v3.Vec{X: 0, Y: 0, Z: 0}))
	// Add space at each end
	spacer := sdf.Transform3D(endSpacer(), sdf.Translate3d(v3.Vec{X: 0, Y: 0, Z: 0}))
	box = sdf.Union3D(box, spacer)
	spacer = sdf.Transform3D(endSpacer(), sdf.Translate3d(v3.Vec{X: 0, Y: 0, Z: length - firstSpace}))
	box = sdf.Union3D(box, spacer)
	...
	for i := 0; i < (numBars - 1); i++ {
		...
		spacer = sdf.Transform3D(spacer, sdf.Translate3d(v3.Vec{X: 0, Y: baseHeight, Z: offset}))
		box = sdf.Union3D(box, spacer)
	}
