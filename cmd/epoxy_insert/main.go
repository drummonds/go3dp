...
func epoxyInsert() (s sdf.SDF3, err error) {
	// var (
	// 	d0, d1 float64
	// )
	// origin, _ := form3.Box(r3.Vec{X: 2, Y: 2, Z: 2}, 0.0)
	// Model lip
	lip, _ := Pipe(lip_height, lip_dia, insert_od, 0)
	tube, _ := Pipe(height, insert_od+wall*2, insert_od, 0)
	insert := sdf.Union3D(tube, lip)
...
func main() {
	...
	// 	log.Fatal(err)
	// }
	err := render.CreateSTL("/mnt/c/t/epoxy_insert.stl", render.NewOctreeRenderer(b, 200))
	if err != nil {
		log.Fatal(err)
	}
}
